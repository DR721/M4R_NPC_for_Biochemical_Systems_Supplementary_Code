...
# --- K-matrix Construction Function ---
def construct_Ks(pp):
    device = pp.device
    zero = lambda: pp[0] * 0
    K_ss = torch.stack([
    ...
    K_sb = pp[30:150].reshape(15,8)
    K_bs = K_sb.T
    K_bb = pp[150:].reshape(8,8)
...
for r in range(repeats):
    ...
    for _ in range(num_epochs):
        ...
        K_ss, K_sb, K_bs, K_bb = construct_Ks(predicted_parameters)
        ode = MyODE(K_ss, K_sb, K_bs, K_bb)
    ...
    values3 = [t.item() for t in parameters_ts[-1][30:150]]
    values4 = [t.item() for t in parameters_ts[-1][150:]]

    zipped = list(zip_longest(values1, values2, values3, values4, fillvalue=None))
    df = pd.DataFrame(zipped, columns=["List 1", "List 2", "List 3", "List 4"])
    df.to_csv(f"_kbb_{scale[22]}_{num_epochs}_{r}.csv")