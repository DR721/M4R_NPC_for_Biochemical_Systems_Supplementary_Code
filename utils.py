...
def EulerMaruyamaMat(*,
    ...
    A = torch.stack([torch.stack([cfg[f'A_{i},{j}'] for j in range(dim)]) for i in range(dim)])
    sigma = cfg["sigma"]

    for _ in range(num_steps):
        noise = sigma * torch.normal(0.0, 1.0, size=data[-1].shape) * np.sqrt(dt)
        drift = torch.matmul(data[-1], A.T) * dt
...
def plot(ts: torch.Tensor,
    ...
    var_names = [f'Var {i}' for i in range(num_vars)] if var_names == 'empty' else var_names
